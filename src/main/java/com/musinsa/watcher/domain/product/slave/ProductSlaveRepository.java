...
public interface ProductSlaveRepository extends JpaRepository<Product, Long> {
  ...
  @Query(value =
      "SELECT p1.product_id, p1.product_name, p1.brand, min(p2.real_price), p1.img, p1.modified_date, \n"
          + "(CASE WHEN  p2.created_date <  ?2 THEN  p2.real_price END - min(p2.real_price)) as discount, \n"
          + "((CASE WHEN  p2.created_date <  ?2  THEN p2.real_price END) - min(p2.real_price))/max(real_price)*100 as percent\n"
